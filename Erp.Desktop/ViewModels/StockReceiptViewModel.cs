...
public sealed partial class StockReceiptViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private ObservableCollection<ReceiptLineViewModel> lines = new();

    [ObservableProperty]
    [NotifyCanExecuteChangedFor(nameof(RemoveLineCommand))]
    private ReceiptLineViewModel? selectedLine;
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            var command = new ReceiveStockCommand
            {
                ...
                Lines = Lines.Select(line => new ReceiveStockLineCommand
                {
            ...
            var result = await _inventoryCommandService.ReceiveStockAsync(command);
            LastTxNo = result.TxNo;
            StatusMessage = $"입고 저장 완료 (TxNo: {result.TxNo})";

            Lines = new ObservableCollection<ReceiptLineViewModel> { new() };
        }
        catch (Exception ex)
        {
